...
namespace MaxiShop.Application.Services
{
    public class PaginationService<T, S> : IPaginationService<T, S> where T : class
    {
        ...
        public PaginationVM<T> GetPagination(List<S> source, PaginationInputModel pagination)
        {
            ...
            var totalPages = (int)Math.Ceiling(totalNoRecords / (double)pageSize);
            var result = source
            ...
            PaginationVM<T> paginationVM=new PaginationVM<T>(currentPage, totalPages, pageSize, totalNoRecords, items);
            return paginationVM;
        }
    }
}
